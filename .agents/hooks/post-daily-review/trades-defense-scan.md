...
trigger:
  ...
  n: 1
script: .venv/Scripts/python.exe scripts/trades_defense_scan.py
output_to: journal
alert_prefix: "🛡️ 持倉防守"
---
...
- 有 `⚠️` 行 → 更新戰術指南 P2（加入減碼 / 觀察待辦）
- 無 `⚠️` 行 → 只寫日誌，不動戰術指南

> `⚠️` 是 daily-review 的落地傳輸格式，不是持續性資料來源。持續性必須讀 `journals/logs/signal_state.json` 的 `action_tag` / `quality`，不得解析 stdout 或 `_state.json.last_output`。

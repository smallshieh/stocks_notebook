...
trigger:
  ...
  n: 1
script: .venv/Scripts/python.exe scripts/wave_decay_alert.py --code 2330 --name 台積電 --alert-wave 0 --context "第一波減持待命（≥ 2,297 元）：三大訊號確認 → 賣 3 股波段倉"
output_to: journal
...
**預計存續**：波段倉 7 股清空後停用（加底線前綴 `_wave-decay-2330.md`）。
